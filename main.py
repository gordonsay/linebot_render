...
請依照使用者輸入的文字，判斷其意圖，並輸出一個 JSON。
重點：請分析使用者的「語氣」與「真實意圖」，不要僅僅因為出現關鍵字就觸發指令。
...
- HELP_COMMANDS
  使用者明確詢問功能列表，如「狗蛋會做什麼」、「指令有哪些」、「教我用」。

- WEATHER_NOW
  問現在天氣。params.city 放城市名（如台北、竹北）。找不到城市則 null。

- WEATHER_FORECAST
  問未來預報。

- GENERATE_IMAGE
  明確要求「畫圖」、「生成圖片」。params.prompt 放描述。
...
- WEB_SEARCH
  要求搜尋網路資訊、Google 某事。params.search_query 放關鍵字。

- PERSON_INTRO
  要求介紹某個人物。params.person_name 放人名。

- IMAGE_SEARCH
  想看圖片、找圖、搜照片。params.image_query 放搜尋關鍵字。

  【關於「色圖」的特殊處理規則】：
  1. 觸發條件：出現「色圖、色色圖、福利圖、香圖、辣妹圖」等詞，且**沒有**出現「影片、看片、番號、AV」等字眼。
  2. **關鍵字多樣化（重要）**：
     為了避免每次搜尋結果都一樣，請**不要**使用固定的字串。
     請根據上下文，從以下詞彙中**隨機挑選或組合**出安全的搜尋詞：
     「性感、比基尼、泳裝、寫真、女神、好身材、模特兒、美腿、甜美、氣質」。
     範例：
     - 使用者：「我要看色圖」 -> params.image_query = "性感 比基尼 女神 寫真" (由你組合，不要每次都一樣)
     - 使用者：「找 XXX 的色圖」 -> params.image_query = "XXX 性感 泳裝 寫真"

- PLAY_SONG
  想聽歌。params.song_name 放歌名。

- TRANSLATE
  要求翻譯並唸出。params.text 放要翻譯的內容。

- AV_DRIVE_SEARCH
  【嚴格觸發】只有出現「開車、看片、AV、番號、無碼、有碼、A片」等明確指涉**影片**的詞彙才使用此 Intent。
  params.keywords 放關鍵字。
  注意：若同時出現「照片」與「AV」，以 AV 為主。
...
- AV_PUSH_SEARCH
  要求推薦符合特定條件的片（推一部 XXX 的片）。

- FIND_STORE
  找餐廳、店家。params.location_name 放店名或地點。

- THEATER
  查詢劇場、電影、動畫集合。

- EMO_REPLY
  【嚴格觸發】只有當使用者**明確要求**狗蛋進行「情勒」或「隨機回應」時才觸發。
  例如：「狗蛋情勒我」、「開啟情勒模式」、「來點情勒語錄」。
  ❌ 排除狀況：若使用者只是說「狗蛋愛你」、「狗蛋不要情勒我」、「你是不是在情勒」，這屬於閒聊，請歸類為 UNKNOWN。
...
- SHOW_ID
  查詢 User ID 或 Group ID。

- UNKNOWN
  不屬於上述任何明確指令，或屬於「閒聊」、「質疑」、「感嘆」性質的對話。
  
  【語氣判斷規則（重要）】：
  如果使用者的語句像是：
  - 「真假你會這個？」(質疑/驚訝) -> UNKNOWN
  - 「亂下指令」(評論/陳述) -> UNKNOWN
  - 「狗蛋你好棒」(稱讚) -> UNKNOWN
  - 「狗蛋愛你」(表達愛意) -> UNKNOWN
  只有在使用者是**「命令句」**或**「明確疑問句」**（如：幫我查...、我要看...、...在哪裡？）時，才歸類為特定 Intent。

=== 輸出限制 ===
1. 僅回傳 JSON 格式。
2. 嚴格遵守欄位定義，無用欄位設為 null。
"""
