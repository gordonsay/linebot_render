...
def handle_message(event):
    ...
    if event.message.type == "image":
        ...
        response = model.generate_content(
            [
                {"text": "請以繁體中文，用朋友聊天的語氣，輕鬆自然、簡短地描述這張照片的內容。語氣請保持冷靜友善，不浮誇、不驚嘆，不使用「哇」、「好棒」之類強烈語氣。不要提出任何問題。不要使用疑問句。只需要像朋友看到照片時說的一句自然描述，並用一句短句帶出你理解的氛圍或可能的感覺即可。例如：
                    - 人物照片可簡單描述狀態，如「看起來蠻悠閒的，都繼續爽不上班沒關係。」
                    - 場景照片可自然描述氣氛，如「整體感覺蠻安靜的，挺適合放鬆, 我也想去廢, 大神帶一下。」
                    - 日常物品可平淡描述用途或氛圍，如「看起來是剛整理好的桌面，很順眼, 我家就是一團垃圾廠, 需要一個家政婦++。」
                    務必簡短、自然，不問問題、不過度解讀、不過度熱情、可以加一點屁話。"},
